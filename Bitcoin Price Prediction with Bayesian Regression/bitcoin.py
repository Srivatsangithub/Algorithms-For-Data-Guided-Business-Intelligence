...
def computeDelta(wt, X, Xi):
    ...
    """
    # YOUR CODE GOES HERE
    # The last value in X must be dropped must be dropped i.e train2_* and yi value from each row of train1_* is needed for computation of Eemp 
    new_X = X[:-1]  # since it is a series we can remove the last value using indexing
    sum_nr = 0.0
    sum_dr = 0.0

    # for each row in the xi df with the given row or train2_*
    for i in range(0, len(Xi)):
      
      # The numerator of Eemp
      similarity_val = similarity_func(new_X, Xi.iloc[i][0:-1])
      similarity_val = wt * similarity_val 
      sum_nr = sum_nr +  (  (Xi.iloc[i][-1]) *  math.exp(similarity_val)  )

      # The denominator part
      sum_dr = sum_dr + math.exp(similarity_val)
    # Estimation val
    Eemp = sum_nr/sum_dr
...
def similarity_func(x, xi):
  # Computing similarity between the two vectors sent: x and x1
  # Need to find the mean and standard deviation of x and xi vector(pandas series) and subtract those values from 
  # each value in the vectors. 
  mean_x = np.mean(x)
  std_x = np.std(x)
  mean_xi = np.mean(xi)
  std_xi = np.std(xi)
  denominator = len(x) * std_x * std_xi

  # Need to loop through the vectors and subtract mean from each values in both the vectors
  sum = 0.0
  for i in range(0, len(x)):
    sum = sum + (  (x[i] - mean_x) * (xi[i] - mean_xi)  )

  similarity_value = sum/denominator
